...
{
    float4 color_in = colorTexture.sample(colorTextureSmplr, colorTexCoord, level(0.0));
    float4x4 color_matrix = float4x4(float4(filterParams0), float4(filterParams1), float4(filterParams2), float4(filterParams3));
    float4 color_out = (color_matrix * color_in) + filterParams4;
    return color_out;
}
...
float3 compositeRGBToHSL(thread const float3& rgb)
{
    ...
    else
    {
        ...
        if (rgb.y == v)
        {
            _992 = float3(2.0, rgb.zx);
        }
        else
        {
            _992 = float3(4.0, rgb.xy);
        }
        _979 = _992;
    }
    float3 terms = _979;
...
kernel void main0(constant int2& uFramebufferTileSize [[buffer(3)]], constant int& uLoadAction [[buffer(4)]], constant int2& uTextureMetadataSize [[buffer(7)]], constant float2& uFramebufferSize [[buffer(0)]], constant float2& uTileSize [[buffer(1)]], constant float4& uClearColor [[buffer(5)]], constant float2& uColorTextureSize0 [[buffer(8)]], constant float2& uMaskTextureSize0 [[buffer(9)]], const device bFirstTileMap& _1603 [[buffer(2)]], const device bTiles& _1694 [[buffer(6)]], texture2d<float, access::read_write> uDestImage [[texture(0)]], texture2d<float> uTextureMetadata [[texture(1)]], texture2d<float> uColorTexture0 [[texture(2)]], texture2d<float> uMaskTexture0 [[texture(3)]], texture2d<float> uGammaLUT [[texture(4)]], sampler uTextureMetadataSmplr [[sampler(0)]], sampler uColorTexture0Smplr [[sampler(1)]], sampler uMaskTexture0Smplr [[sampler(2)]], sampler uGammaLUTSmplr [[sampler(3)]], uint3 gl_WorkGroupID [[threadgroup_position_in_grid]], uint3 gl_LocalInvocationID [[thread_position_in_threadgroup]])
{
    ...
    int tileIndex = _1603.iFirstTileMap[tileCoord.x + (uFramebufferTileSize.x * tileCoord.y)];
    ...
    while (tileIndex >= 0)
    {
        for (int subY_1 = 0; subY_1 < 4; subY_1++)
        {
            ...
            int alphaTileIndex = int(_1694.iTiles[(tileIndex * 4) + 2] << uint(8)) >> 8;
            uint tileControlWord = _1694.iTiles[(tileIndex * 4) + 3];
        ...
        tileIndex = int(_1694.iTiles[(tileIndex * 4) + 0]);
    }
