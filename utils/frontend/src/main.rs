...
fn convert_font(font_path: &Path, output_path: &Path) -> Result<(), ()> {
    ...
    for glyph_index in 0..glyph_count {
        ...
        if font
            .outline(glyph_index, HintingOptions::None, &mut path_builder)
            .is_err()
        {
            continue;
        }
...
pub fn main() {
    let app = App::new("Pathfinder Build Utility")
        ...
        .arg(
            Arg::with_name("FONT-PATH")
                .help("The `.ttf` or `.otf` font file to use")
                .required(true)
                .index(1),
        )
        .arg(
            Arg::with_name("OUTPUT-PATH")
                .help("The `.pfml` mesh library to produce")
                .index(2),
        );
    ...
    let output_path = match matches.value_of("OUTPUT-PATH") {
        ...
        None => match font_path.file_stem() {
            None => {
