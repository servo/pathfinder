...
impl ColorMatrix {
    ...
    /// Create a ` hueRotate` ColorMatrix filter
    /// 
    /// Note that the paramete is in radians.
    /// 
    /// (See the `feColorMatrix` element in SVG specification.)
    pub fn hue_rotate(rotaion: f32) -> ColorMatrix {
        ...
        a + b * rotaion.cos() + c * rotaion.sin()
    }

    /// Create a `saturate` ColorMatrix filter 
    /// 
    /// (See the `feColorMatrix` element in SVG specification.)
    ...
    /// Create a `luminanceToAlpha` ColorMatrix filter
    /// 
    /// (See the `feColorMatrix` element in SVG specification.)
